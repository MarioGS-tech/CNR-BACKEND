...
public interface MenuItemsRepository extends JpaRepository<MenuItems, Long> {
    ...
    @Override
    @Query(value = "SELECT m FROM MenuItems m WHERE m.deletedAt IS NULL", countQuery = "SELECT COUNT(m) FROM MenuItems m WHERE m.deletedAt IS NULL")
    Page<MenuItems> findAll(Pageable page);

    @Query(value = "SELECT m FROM MenuItems m WHERE m.deletedAt IS NULL AND m.idMenu = :id", countQuery = "SELECT COUNT(m) FROM MenuItems m WHERE m.deletedAt IS NULL AND m.idMenu = :id")
    Page<MenuItems> findAllByMenuId(Long id, Pageable page);
