...
public interface TransaccionRepository extends JpaRepository<Transaccion, Long>, PagingAndSortingRepository<Transaccion, Long>, JpaSpecificationExecutor<Transaccion> {
    ...
    Page<Transaccion> findTransaccionesByClienteNombre(
            @Param("nombreCliente") String nombreCliente,
            Pageable pageable
    );

    @Query("SELECT t FROM Transaccion t WHERE t.status = :status AND t.deletedAt IS NULL")
    Page<Transaccion> findTransaccionesByStatusDte(
            @Param("status") EstatusCola nombreCliente,
            Pageable pageable
    );

    @Query("SELECT t FROM Transaccion t WHERE t.totalTransaccion = :total AND t.deletedAt IS NULL")
    Page<Transaccion> findTransaccionesByTotalTransaccion(
            @Param("total") BigDecimal total,
            Pageable pageable
    );

    @Query(value = "SELECT t FROM Transaccion t WHERE t.deletedAt IS NULL", countQuery = "SELECT COUNT(s) FROM Transaccion s WHERE s.deletedAt IS NULL")
    ...
    @Query("SELECT s FROM Transaccion s INNER JOIN Cliente c ON c.idCliente = s.cliente.idCliente WHERE s.tipoDTE IN :tipoDTE AND s.cliente.nit = :nit AND s.deletedAt IS NULL AND s.status = 2 ORDER BY s.idTransaccion DESC")
