...
public class UserController {
    ...
    public ResponseEntity<Object> saveUser(@RequestBody User user) {
        try {
            // TEMPORAL ELIMINAR AL INTEGRAR API DE USUARIO
            user.setPassword(BCrypt.hashpw(pass, BCrypt.gensalt()));
            return utils.jsonResponse(200, "Se agrego el usuario exitosamente", userService.saveUser(user));
        } catch (Exception ex) {
            return utils.jsonResponse(500, "No se agrego el usuario", ex.getMessage());
        }
    }
    ...
    public ResponseEntity<Object> cambiarSisucc(@RequestBody AuthenticationRequest request) {
        try {
            return utils.jsonResponse(200, "Se ha cambiado la contraseña", authenticationService.cambiarSisucc(request));
        } catch (Exception ex) {
            return utils.jsonResponse(500, "No se pudo cambiar la contraseña", ex.getMessage());
